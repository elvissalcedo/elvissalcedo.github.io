...
- **TODA fórmula o cálculo va SIEMPRE en LaTeX real** -- es el único formato de fórmulas que reconoce el sitio (Jekyll/kramdown), y MathJax (cargado en el sitio) lo renderiza en el navegador.
  ...
  - **Si usas `\begin{align*}...\end{align*}` para alinear varias líneas de una ecuación: el separador de fila DEBE ser `\\\\` (cuatro backslashes) en vez de `\\` (dos), y todo el bloque `align*` va en una sola línea física de texto, sin saltos de línea reales entre las filas.** Kramdown convierte un salto de línea real después de uno o varios backslashes en un `<br>` que rompe la fórmula -- no importa cuántos backslashes pongas, si hay un Enter real justo después, kramdown se come toda la corrida igual. Separa las filas visualmente solo con el `\\\\` (cuatro backslashes), nunca con Enter.
  - **El asterisco de `align*` (`\begin{align*}`) puede leerse como cursiva de Markdown** si los dos asteriscos -- el de `\begin{align*}` y el de `\end{align*}` -- quedan pegados de cierta forma dentro del mismo párrafo: Kramdown los toma como un par de énfasis que abarca todo el bloque de por medio. Escápalo siempre como `\begin{align\*}` y `\end{align\*}`, con el backslash antes del asterisco.
...
- ¿Si usaste `\begin{align*}`, todo el bloque está en una sola línea física, con `\\\\` (4 backslash) como separador de fila, y el asterisco escapado (`align\*`)?
